...
Raw reads were demultiplexed and filtered using *process_radtags* in Stacks 2.66 (Rochette et al., 2019). Demultiplexed reads were processed and assembled using the *denovo_map.pl* pipeline in Stacks. Parameters were determined following Paris et al. (2017). Assembled 3RAD loci were filtered using *populations* within Stacks.
...
For each lineage, observed heterozygosity (Ho), expected heterozygosity (He), nucleotide diversity (π), and inbreeding coefficients (FIS) were calculated by *populations* using the lineage-specific multiple SNPs per locus datasets. FST was also calculated using the Weir and Cockerham (1984) method, implemented in the R package HIERFSTAT (Goudet, 2005), to allow for comparisons among values calculated in other studies. 
...
An analysis of molecular variance (AMOVA; Excoffier et al., 1992) was conducted using the multiple SNPs per locus dataset for each lineage to test for population structure among collection sites. Using the poppr.amova function in the R package ADEGENET (Jombart, 2008; Kamvar et al., 2014), individuals were stratified by collection site. Significance was tested with a 999-permutation randomized test.
...
scatter(dapc2, solid=0.4, cex=3, 
        ratio.da = 0.2,
        posi.pca = "topleft", scree.pca = TRUE, ratio.pca=0.3,
...
Gent.Metadata <- read.csv("05_Population_Structure_Admixture/Geniculata_Metadata2.csv", header = TRUE, na.strings = "NA")
...
# Best fit K-value is 2.
Ful.qmatrix.K5 <- Admixture.Lea.K.qmatrix(Ful.input.file, 5)
```

```{r 05_LEA_Admixture_Analysis L. fuliginosa P2, echo=TRUE}
# Convert Vector into DataFrame
Ful.qmatrix.K5 <- data.frame(Ful.qmatrix.K5)

# Add Specimen Column from Metadata to Qmatrix for Future Joining
Ful.qmatrix.K5$Specimen <- Ful.Metadata$Specimen

# Combine with Metadata DataFrame
Ful.LEA.K5.Join <- full_join(Ful.Metadata, Ful.qmatrix.K5, by = "Specimen")

# Convert from Wide to Long Format
Ful.LEA.K5.Long <- Ful.LEA.K5.Join %>% 
  pivot_longer(
    cols = `V1`:`V5`, 
    names_to = "Admixure",
    values_to = "Amount"
...
# Convert the Column Site into a Factor to Allow for Faceting
Ful.LEA.K5.Long$Site <- as.factor(Ful.LEA.K5.Long$Site)

# Using Levels, Reorder Sites so They Run Downstream to UpStream
Ful.LEA.K5.Long$Site <- factor(Ful.LEA.K5.Long$Site, levels = c("D",
                                                                    "SP",
                                                                    "FW",
                                                                    "SM",
                                                                    "C",
...
# Graph the Admixture Results. Use Facet to Split into Sections by Site
Ful.admixture <- ggplot(Ful.LEA.K5.Long, aes(x = factor(Specimen), y = Amount, fill = Admixure)) +
  geom_col(width = 1) +
  # Facet over the groups
  facet_grid(~ Site, switch = "x", scales = "free", space = "free") +
  # Use these expand arguments to get rid of padding around the axes
...
Gent.qmatrix.K3 <- Admixture.Lea.K.qmatrix(Gent.input.file, 3)
```

```{r 05_LEA_Admixture_Analysis L. geniculata P2, echo=TRUE}
# Convert Vector into DataFrame
Gent.qmatrix.K3 <- data.frame(Gent.qmatrix.K3)

# Add Specimen Column from Metadata to Qmatrix for Future Joining
Gent.qmatrix.K3$Specimen <- Gent.Metadata$Specimen

# Combine with Metadata DataFrame
Gent.LEA.K3.Join <- full_join(Gent.Metadata, Gent.qmatrix.K3, by = "Specimen")

# Convert from Wide to Long Format
Gent.LEA.K3.Long <- Gent.LEA.K3.Join %>% 
  pivot_longer(
    cols = `V1`:`V3`, 
    names_to = "Admixure",
    values_to = "Amount"
...
# Convert the Column Site into a Factor to Allow for Faceting
Gent.LEA.K3.Long$Site <- as.factor(Gent.LEA.K3.Long$Site)

# Using Levels, Reorder Sites so They Run Downstream to UpStream
Gent.LEA.K3.Long$Site <- factor(Gent.LEA.K3.Long$Site, levels = c("Dyer",
                                                                      "NV",
                                                                      "CV",
                                                                      "TB",
                                                                      "LL",
...
# Graph the Admixture Results. Use Facet to Split into Sections by Site
Gent.admixture <- ggplot(Gent.LEA.K3.Long, aes(x = factor(Specimen), y = Amount, fill = Admixure)) +
  geom_col(width = 1) +
  # Facet over the groups
  facet_grid(~ Site, switch = "x", scales = "free", space = "free") +
  # Use these expand arguments to get rid of padding around the axes
...
Gent.admixture
```
### 06 Genetic CoAncestry - fineRADstructure
Run on Linux, Graph in R.
...
### 4) SET VALUES FOR THESE VARIABLES: "analysisName" will be included in output plots
Dut.analysisName <- "Duttoniana_Genetic_CoAncestry";  maxIndv <- 1000; maxPop<-1000
